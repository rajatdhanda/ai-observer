...
class EntityIdentifier {
    async identify(types, dataFlow) {
        ...
        const modelTypes = types.interfaces.filter(t => this.looksLikeEntity(t.name) && t.category !== 'database');
    ...
    looksLikeEntity(name) {
        ...
        return entityPatterns.some(pattern => name.toLowerCase().includes(pattern.toLowerCase()));
    }
    createEntity(type, types) {
    ...
    establishRelationships(entities, types) {
        for (const entity of entities) {
            for (const prop of entity.properties) {
                ...
                if (prop.name.endsWith('_id') || prop.name.endsWith('Id')) {
                    ...
                    const relatedEntity = entities.find(e => e.name.toLowerCase() === relatedEntityName.toLowerCase());
                    if (relatedEntity) {
        ...
        for (const entity of entities) {
            for (const rel of entity.relationships) {
                if (rel.type === 'belongsTo') {
                    ...
                    if (relatedEntity) {
                        const inverseExists = relatedEntity.relationships.some(r => r.entity === entity.name && r.type === 'hasMany');
                        if (!inverseExists) {
    ...
    extractEntityFromForeignKey(propName) {
    ...
    extractEntityFromArrayType(propName) {
    ...
    identifyOperations(entities, dataFlow) {
        for (const entity of entities) {
            const operations = [];
            ...
            const relevantApis = apiComponents.filter(api => api.toLowerCase().includes(entity.name.toLowerCase()));
