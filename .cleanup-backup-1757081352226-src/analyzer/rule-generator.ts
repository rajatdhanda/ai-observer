...
export class ValidationRuleGenerator {
  ...
  private generateTypeRules(types: TypeSystem): ValidationRule[] {
    const rules: ValidationRule[] = [];
  ...
  private generateBusinessRules(entities: BusinessEntity[]): ValidationRule[] {
    const rules: ValidationRule[] = [];
    
    for (const entity of entities) {
      // Email validation
      const emailProp = entity.properties.find(p => 
      ...
      // Phone validation
      const phoneProp = entity.properties.find(p => 
      ...
      // Date validation
      const dateProp = entity.properties.find(p => 
      ...
      const priceProp = entity.properties.find(p => 
  ...
  private generateDataFlowRules(dataFlow: DataFlowMap): ValidationRule[] {
    const rules: ValidationRule[] = [];
  ...
  private generatePerformanceRules(entities: BusinessEntity[]): ValidationRule[] {
    const rules: ValidationRule[] = [];
  ...
  private generateSecurityRules(entities: BusinessEntity[], types: TypeSystem): ValidationRule[] {
    const rules: ValidationRule[] = [];
    
    // Password validation
    for (const entity of entities) {
      const passwordProp = entity.properties.find(p => 
