...
class TableMapper {
    ...
    async discoverTables() {
        ...
        if (typesPaths.length === 0) {
            ...
            const typeFiles = allFiles.filter(f => f.includes('.types.ts') ||
                f.includes('/types/') ||
    ...
    async extractTablesFromFile(file) {
            ...
            for (const file of files) {
                ...
                for (const [tableName, table] of this.tables) {
                    ...
                    if (patterns.some(p => p.test(source))) {
                        const operations = [];
                        if (/\.select|\.find|\.get|SELECT/i.test(source))
                            operations.push('select');
                        if (/\.insert|\.create|INSERT/i.test(source))
                            operations.push('insert');
                        if (/\.update|\.upsert|UPDATE/i.test(source))
                            operations.push('update');
                        if (/\.delete|\.remove|DELETE/i.test(source))
    ...
    findComponentsUsingHook(hookName) {
        const components = [];
            ...
            for (const file of files) {
                ...
                for (const [tableName, table] of this.tables) {
                    ...
                    if (source.toLowerCase().includes(tableName)) {
                        const operations = [];
                        if (/use.*Query|fetch|get|list/i.test(hookName))
                            operations.push('fetch');
                        if (/use.*Create|useAdd/i.test(hookName))
                            operations.push('create');
                        if (/use.*Update|useEdit/i.test(hookName))
                            operations.push('update');
                        if (/use.*Delete|useRemove/i.test(hookName))
                            operations.push('delete');
                        if (/use.*Mutation/i.test(hookName))
        ...
        for (const apiPath of apiPaths) {
            ...
            for (const file of files) {
                ...
                for (const [tableName, table] of this.tables) {
                    if (source.toLowerCase().includes(tableName)) {
                        const methods = [];
                        if (/export.*GET|async.*GET/i.test(source))
                            methods.push('GET');
                        if (/export.*POST|async.*POST/i.test(source))
                            methods.push('POST');
                        if (/export.*PUT|async.*PUT/i.test(source))
                            methods.push('PUT');
                        if (/export.*DELETE|async.*DELETE/i.test(source))
                            methods.push('DELETE');
                        if (/export.*PATCH|async.*PATCH/i.test(source))
                            methods.push('PATCH');
        ...
        for (const [tableName, table] of this.tables) {
            for (const hook of table.hooks) {
                ...
                if (source.includes('useMutation')) {
                    const hasCacheInvalidation = source.includes('invalidateQueries') ||
    ...
    calculateScores() {
        for (const [tableName, table] of this.tables) {
            ...
            if (table.hooks.length > 0) {
                score += 10;
                if (table.hooks.some(h => h.hasErrorHandling && h.hasLoadingState))
                    score += 10;
            }
            ...
            if (table.mutations.length > 0) {
                maxScore += 10;
                if (table.mutations.some(m => m.hasCacheInvalidation))
                    score += 10;
            }
