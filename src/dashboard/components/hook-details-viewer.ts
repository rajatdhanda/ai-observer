...
export class HookDetailsViewer {
  ...
  render(hookName: string, hookData: any, validationData?: any): void {
  ...
  private extractHookData(name: string, hookData: any): HookData {
  ...
  private getViolations(hookName: string, validationData: any): any[] {
    const violations = [];
  ...
  private extractLineNumber(location: string): number | null {
    const match = location.match(/:(\d+)$/);
    return match ? parseInt(match[1], 10) : null;
  }
  
  private calculateHealthScore(data: HookData, violations: any[]): number {
    let score = 100;
  ...
  private renderHeader(data: HookData): string {
    return `
  ...
  private renderHealthScore(score: number, data: HookData, violations: any[]): string {
  ...
  private renderConfiguration(data: HookData): string {
    return `
      <div style="
        background: #1a1a1a;
  ...
  private renderIssues(data: HookData, violations: any[]): string {
    const issues = [];
  ...
  private renderUsage(data: HookData): string {
    if (!data.usedInComponents || data.usedInComponents.length === 0) {
      return `
        <div style="
          background: #f59e0b10;
  ...
  private renderErrorChain(data: HookData): string {
  ...
  private renderRecommendations(data: HookData, violations: any[]): string {
