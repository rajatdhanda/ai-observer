...
export class DesignSystemValidator {
  ...
  private allowedDSPaths: string[] = [];
  private detectedDSPath: string = '';
  ...
  private checkImportGuard(filePath: string, content: string, lines: string[]) {
  ...
  private checkTokenGuard(filePath: string, content: string, lines: string[]) {
  ...
  private checkComponentUsage(filePath: string, content: string, lines: string[]) {
  ...
  private checkA11yGuard(filePath: string, content: string, lines: string[]) {
    ...
    for (const match of inputMatches) {
      ...
      const nearbyContent = content.substring(
        ...
        Math.min(content.length, (match.index || 0) + 200)
      );
  ...
  private getLineIndex(content: string, charIndex: number): number {
  ...
  private groupByType(violations: DesignViolation[]): Record<string, DesignViolation[]> {
    const grouped: Record<string, DesignViolation[]> = {};
